...
const BlogContent = ({blogs}) => {
  ...
  let blog = blogs.data.filter((blog) => blog.id == id)[0];


  return (


    <div className="w-full py-32 sm:py-10 bg-[#f9f9f9]">
      <div className="max-w-[1240px] mx-auto">
        <div
          className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 ss:grid-cols-1 xs:grid-cols-1
            md:gap-x-8 sm:gap-y-8 ss:gap-y-8 px-4 sm:pt-20 md:mt-0 ss:pt-20 text-black"
        >
          <div className="col-span-2 ">
            <img className="h-56 w-full object-cover rounded" src={`http://localhost:1337${blog.attributes.coverImg.data.attributes.url}`}  />
